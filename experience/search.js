...
var Cmdk=(function(){
  ...
  function exec(){var it=res[sel];if(!it)return;pushRecent(it);trigger=null;close();if(it.k==='view')go(it.id);else go(it.k,it.id);var h=main.querySelector('h1');if(h){h.tabIndex=-1;h.focus({preventScroll:true});}}
  function mountEl(){el=ce('dialog');el.id='cmdk';el.setAttribute('aria-label','Search Physics Entelloq');
    ...
    document.body.appendChild(el);inp=$('#cp-in');list=$('#cp-list');inp.addEventListener('input',refresh);$('#cp-close').addEventListener('click',close);$('#cp-ask').addEventListener('click',function(){var q=inp.value.trim();if(q){trigger=null;close();Tutor.ask(q);}});
    ...
    el.addEventListener('keydown',function(e){if(e.key!=='Tab')return;var items=$$('input,button',el).filter(function(n){return !n.disabled&&n.getClientRects().length;}),first=items[0],last=items[items.length-1];if(e.shiftKey&&document.activeElement===first){e.preventDefault();last.focus();}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus();}});
  }
  ...
  function close(){if(el&&el.open){el.classList.remove('on');inp.setAttribute('aria-expanded','false');el.close();if(trigger&&trigger.isConnected)trigger.focus({preventScroll:true});}}
