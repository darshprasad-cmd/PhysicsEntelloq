...
(async()=>{const browser=await chromium.launch({headless:true});try{
 for(const [device,width,height] of [['desktop',1440,1000],['mobile',390,844],['small',320,740]]){
  ...
  await page.locator('[data-signup]:visible').first().click();await page.locator('#pe-account-dialog[open]').waitFor();await scan('account','#pe-account-dialog');
  ...
  await page.locator('[data-signup]:visible').first().click();
