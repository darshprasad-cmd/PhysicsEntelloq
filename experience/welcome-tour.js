...
var WelcomeTour=(function(){
  ...
  function setPlaying(value){playing=!!value;last=performance.now();if(dialog){$('#welcome-play').textContent=playing?'Pause':'Play walkthrough';$('#welcome-play').setAttribute('aria-pressed',String(playing));$('#welcome-play').setAttribute('aria-label',playing?'Pause walkthrough':'Play walkthrough');$('#welcome-timing').textContent=playing?'Auto-playing · pauses when you interact':'At your pace · use Next or press Play';}stopTimer();if(playing)timer=setInterval(tick,200);}
  ...
  function position(){if(!dialog||!dialog.open)return;var target=document.querySelector(steps[index].target),r=target&&target.getBoundingClientRect();if(!r||r.width<1||r.height<1)r={left:20,top:80,width:Math.max(0,innerWidth-40),height:1,right:innerWidth-20,bottom:81};
    var left=Math.max(6,r.left-7),top=Math.max(6,r.top-7),width=Math.max(0,Math.min(innerWidth-6,r.right+7)-left),height=Math.max(0,Math.min(innerHeight-6,r.bottom+7)-top);
    Object.assign($('#welcome-highlight').style,{left:left+'px',top:top+'px',width:width+'px',height:height+'px'});
    var card=$('#welcome-card'),mobile=innerWidth<=700;card.classList.toggle('welcome-card-top',mobile&&r.top>innerHeight*.52);card.classList.toggle('welcome-card-left',!mobile&&r.left>innerWidth*.7&&r.top>innerHeight*.5);
  }
  function render(next,manual){index=Math.max(0,Math.min(steps.length-1,next));remaining=11000;last=performance.now();var s=steps[index];go(s.route);WelcomeState.write(user,'active',index);
    $('#welcome-count').textContent='STEP '+(index+1)+' OF '+steps.length;$('#welcome-hint').textContent=s.hint;$('#welcome-title').textContent=s.title;$('#welcome-copy').textContent=s.text;$('#welcome-next').textContent=index===steps.length-1?'Start exploring →':'Next →';$('#welcome-back').disabled=index===0;$('#welcome-meter').value=0;
    $('#welcome-dots').innerHTML=steps.map(function(_,i){return '<span class="'+(i<=index?'on':'')+'"></span>';}).join('');position();moveFocus();if(manual)setPlaying(false);
  }
