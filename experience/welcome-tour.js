...
var WelcomeTour=(function(){
  ...
  function revealTarget(){var target=document.querySelector(steps[index].target),scroll=$('#main-scroll');if(target&&target.closest&&target.closest('#main')){if(innerWidth<=700){var bar=document.querySelector('.topbar'),edge=Math.max(scroll.getBoundingClientRect().top,bar?bar.getBoundingClientRect().bottom:0);scroll.scrollTop=Math.max(0,scroll.scrollTop+target.getBoundingClientRect().top-edge-24);}else target.scrollIntoView({block:'nearest',behavior:'instant'});}position();}
